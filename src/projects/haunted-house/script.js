...
const bushColorTexture = textureLoader.load(
  "./assets/bush/leaves_forest_ground_1k/leaves_forest_ground_diff_1k.webp",
);
...
const bushARMTexture = textureLoader.load(
  "./assets/bush/leaves_forest_ground_1k/leaves_forest_ground_arm_1k.webp",
);
bushARMTexture.repeat.set(2, 1);
bushARMTexture.wrapS = THREE.RepeatWrapping;

const bushNormalTexture = textureLoader.load(
  "./assets/bush/leaves_forest_ground_1k/leaves_forest_ground_nor_gl_1k.webp",
);
...
door.position.y += 1;
door.position.z += 2.01; // +0.01 because of z-fighting
house.add(door);
...
// Randomizing position and orientation of graves
for (let i = 0; i < 30; i++) {
  /* Method 1 */
  // const directions = [-1, 1];
  // const randomDirectionIndexX = Math.round(Math.random());
  // const directionX = directions[randomDirectionIndexX];
  // const randomDirectionIndexZ = Math.round(Math.random());
  // const directionZ = directions[randomDirectionIndexZ];

  // const randomX = Number((Math.random() * 7 + 2).toFixed(2));
  // const randomZ = Number((Math.random() * 7 + 2).toFixed(2));

  /* Method 2 */
  // Passing same angle to sin and cos gives us co-ordinates that are on a circle
  ...
  const rotateX = (Math.random() - 0.5) * 0.25;
  const rotateY = (Math.random() - 0.5) * 0.25;
  const rotateZ = (Math.random() - 0.5) * 0.25;

  const grave = new THREE.Mesh(graveGeometry, graveMaterial);
  grave.position.set(x, y, z);
  grave.rotation.set(rotateX, rotateY, rotateZ);
  graves.add(grave);
}
...
// Update the parameters of the custom shader
